...
CREATE TABLE IF NOT EXISTS `DEVICE_EVENT` (
    `ID` INT PRIMARY KEY AUTO_INCREMENT,
    `DEVICE_UID` BINARY(16),
    `VERSION` INT,
    `CREATED_DATE` DATETIME,
    `EVENT` JSON
);

CREATE INDEX `DEVICE_EVENT_DEVICE_UID_INDEX` ON `DEVICE_EVENT` (`DEVICE_UID`);
...
CREATE TABLE IF NOT EXISTS `DEVICE_READ` (
    ...
    `STATUS` VARCHAR(255),
    `DOMAIN_CODE` VARCHAR(255),
    `DOMAIN_DATA_DEVICE_ID` BINARY(16),
    `DOMAIN_DATA_AREA_ID` BINARY(16),
    `DOMAIN_DATA_CROP_ID` BINARY(16),
    `CATEGORY` VARCHAR(255),
    `ASSET_ID` BINARY(16)
...
CREATE INDEX `DEVICE_READ_UID_UNIQUE_INDEX` ON `DEVICE_READ` (`UID`);
